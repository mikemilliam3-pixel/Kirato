...
const EducationLayout: React.FC<EducationLayoutProps> = ({ children }) => {
  ...
  const t = eduTranslations[language as keyof typeof eduTranslations] || eduTranslations['EN'];
        ...
        <nav className="max-w-7xl mx-auto w-full flex gap-4 sm:gap-6 overflow-x-auto no-scrollbar md:justify-center md:gap-10 pb-1">
          {eduRoutes.map((route) => {
            ...
            return (
              <button
                key={route.id}
                onClick={() => navigate(`/modules/education/${route.path}`)}
                className={`flex flex-col items-center gap-1.5 pb-3 min-w-[70px] sm:min-w-[90px] md:min-w-0 transition-all relative group shrink-0 ${
                  isActive ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
                }`}
              >
