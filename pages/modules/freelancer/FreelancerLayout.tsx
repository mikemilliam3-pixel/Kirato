...
import { freelancerRoutes } from './routes';
...
const FreelancerLayout: React.FC<FreelancerLayoutProps> = ({ children }) => {
  ...
  const t = freelancerTranslations[language];
        ...
        <div className="max-w-6xl mx-auto w-full flex gap-6 overflow-x-auto no-scrollbar md:overflow-visible md:justify-center md:gap-8 lg:gap-12 px-4 pb-1">
          {freelancerRoutes.map((route) => {
            ...
            return (
              <button
                key={route.id}
                onClick={() => navigate(`/modules/freelancer/${route.path}`)}
                className={`flex flex-col items-center gap-2 pb-3 min-w-[80px] md:min-w-0 transition-all relative ${
