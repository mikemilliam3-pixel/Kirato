            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 pb-3 min-w-[70px] sm:min-w-[90px] md:min-w-0 transition-all relative group shrink-0 ${
                  isActive ? 'text-amber-500' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
                }`}
              >
                ...
                {isActive && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-amber-500 rounded-t-full shadow-[0_-4px_10px_rgba(245,158,11,0.3)]" />
                )}
              </button>
            );
          })}
