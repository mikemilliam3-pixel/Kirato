...
const VoiceLayout: React.FC<VoiceLayoutProps> = ({ children }) => {
        ...
        <nav className="max-w-7xl mx-auto w-full flex gap-4 sm:gap-6 overflow-x-auto no-scrollbar md:justify-center md:gap-10 pb-1">
          {voiceRoutes.map((route) => {
            ...
            return (
              <button
                key={route.id}
                onClick={() => navigate(`/modules/voice/${route.path}`)}
                className={`flex flex-col items-center gap-1.5 pb-3 min-w-[70px] sm:min-w-[85px] md:min-w-0 transition-all relative group shrink-0 ${
                  isActive ? 'text-orange-500' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
                }`}
              >
                ...
                {isActive && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-orange-500 rounded-t-full shadow-[0_-4px_10px_rgba(245,158,11,0.3)]" />
                )}
              </button>
            );
          })}
